...
class Program
{
    ...
    private static Dictionary<int, List<Passenger>> _waitingPassengers = new();
    ...
    private static Dictionary<int, int> _busesArrived = new();
    ...
    static void BusArrival(Bus bus)
    {
        Thread.Sleep(3000);
            ...
            lock (Lock)
            {
                ...
                int taking = Math.Min(waitingCount, bus.Capacity);

                var boardingPassengers = _waitingPassengers[bus.Number].GetRange(0, waitingCount);
                ...
                Console.WriteLine($"Автобус №{bus.Number} забрав {waitingCount} пасажирів. Залишилось на зупинці: {_waitingPassengers[bus.Number].Count}");
